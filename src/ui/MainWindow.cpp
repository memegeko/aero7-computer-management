...
void MainWindow::buildMenus()
{
    ...
    m_back=bar->addAction(QIcon::fromTheme("go-previous"),"Back",this,[this]{navigate(m_history.back(),false);});m_forward=bar->addAction(QIcon::fromTheme("go-next"),"Forward",this,[this]{navigate(m_history.forward(),false);});bar->addSeparator();bar->addAction(QIcon::fromTheme("view-refresh"),"Refresh",this,[this]{if(m_currentPage)m_currentPage->refresh();});bar->addAction(QIcon::fromTheme("help-contents"),"Help",[]{QDesktopServices::openUrl(QUrl("https://github.com/memegeko/aero7-computer-management#readme"));});
}
...
{
    while(auto *item=m_actionsLayout->takeAt(0)){delete item->widget();delete item;}
...
void MainWindow::restoreState(){QSettings s("Aero7","ComputerManagement");restoreGeometry(s.value("geometry").toByteArray());m_splitter->restoreState(s.value("splitter").toByteArray());}
void MainWindow::closeEvent(QCloseEvent *event){QSettings s("Aero7","ComputerManagement");s.setValue("geometry",saveGeometry());s.setValue("splitter",m_splitter->saveState());s.setValue("lastNode",m_history.current());QMainWindow::closeEvent(event);}
